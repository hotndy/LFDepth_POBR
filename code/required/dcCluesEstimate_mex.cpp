...
/* Input Arguments */
#define	X_size_fin			prhs[0]  // spatial image width
#define	Y_size_fin          prhs[1]  // spatial image height
#define	UV_diameter         prhs[2]  // angular resolution
#define	Im_in_remap         prhs[3]  // the remap image
#define Alpha_min           prhs[4]  // minimum disparity between neighboring cameras
#define Alpha_max           prhs[5]  // maximum disparity between neighboring cameras
#define Depth_res           prhs[6]  // depth resolution
//#define Orient              prhs[7]

/* Output Arguments */
// #define	Depth               plhs[0]  // best depth based on summed depth and defocus cost
// #define Depth_var           plhs[1]  // depth variance, 2x 
// #define Depth_var_sum       plhs[2]  // summed depth and defocus cost, 1x
#define Depth_var_corresp   plhs[0]  // correspondence cost
#define Depth_var_defocus	plhs[1]  // defocus cost   
#define Refocus             plhs[2]  // refocused image, 2x
...
void computeDepthCost(float depth_cost[], double im_in_remap[], double im_pinhole[], int x, int y)
{
    double* remap    = new double[uv_size*3];
    // Jake // double* remap_p1 = new double[uv_size*3];
    // Jake // double* remap_p2 = new double[uv_size*3];
...
void costFilter(float depth_cost[], float depth_var_corresp[], float depth_var_defocus[], double im_pinhole[])
{
    for (int i = 0; i < depth_res; i++) {
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                depth_var_corresp[y+x*height+i*pixelNum]= sumVal_c;				
...
void depthEstimate(double* im_in_remap, double* im_pinhole, float* depth_cost, float* depth_var_corresp, float* depth_var_defocus)
{
    // compute depth cost for each depth
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            computeDepthCost(depth_cost, im_in_remap, im_pinhole, x, y);
        }
    }
    
    // apply a bilateral filter on the depth cost
    costFilter(depth_cost, depth_var_corresp, depth_var_defocus, im_pinhole);
...
//  double* orient_pt       = mxGetPr(Orient);
    ...
    // still create it, just not assign it to output pointers
    ...
    // Mine
    const mwSize dims2[] = { height, width, depth_res };
    Depth_var_corresp = mxCreateNumericArray(3, dims2, mxSINGLE_CLASS, mxREAL);
    float* depth_var_corresp_pt = (float*)mxGetPr(Depth_var_corresp);
    
    Depth_var_defocus = mxCreateNumericArray(3, dims2, mxSINGLE_CLASS, mxREAL);
    float* depth_var_defocus_pt = (float*)mxGetPr(Depth_var_defocus);
    // Mine
    ...
    // depthEstimate(im_in_remap_pt, im_pinhole, depth_pt, depth_cost, 
            // depth_var_sum_pt, orient_pt);
    depthEstimate(im_in_remap_pt, im_pinhole, depth_cost, 
            depth_var_corresp_pt, depth_var_defocus_pt);
